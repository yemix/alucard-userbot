...
import discord, subprocess, sys, time, os, colorama, base64, codecs, datetime, io, random, numpy, datetime, smtplib, string, ctypes, youtube_dl, typing
import urllib.parse, urllib.request, re, json, requests, webbrowser, dns, aiohttp, asyncio, functools, logging
...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Alucard UserBot v{USERBOT.__version__}] | Loading...')
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                      ...
                      ░██▄▄▄▄██ ▒██░    ▓▓█  ░██░▒▓▓▄ ▄██▒░██▄▄▄▄██ ▒██▀▀█▄  ░▓█▄   ▌
                       ...
                       {Fore.CYAN}Alucard UserBot {USERBOT.__version__} | {Fore.GREEN}Logged in as: {Alucard.user.name}#{Alucard.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Alucard.user.id}   
                       ...
                       {Fore.CYAN}Creator: | {Fore.GREEN}BombenProdukt#1337
    '''+Fore.RESET)
...
def Init():
    if config.get('token') == "token-here":
        ...
        try:
            ...
            os.system(f'title (Alucard UserBot) - Version {USERBOT.__version__}')
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Alucard UserBot v{USERBOT.__version__}] | Logged in as {Alucard.user.name}')
...
@Alucard.command(pass_context=True, aliases=['j', 'joi'])
@commands.check(is_owner)
async def join(ctx):
    channel = ctx.message.author.voice.channel
    voice = get(Alucard.voice_clients, guild=ctx.guild)
...
@Alucard.command(pass_context=True, aliases=['l', 'lea'])
@commands.check(is_owner)
async def leave(ctx):
    channel = ctx.message.author.voice.channel
    voice = get(Alucard.voice_clients, guild=ctx.guild)
...
@Alucard.command(pass_context=True, aliases=['p', 'pla'])
@commands.check(is_owner)
